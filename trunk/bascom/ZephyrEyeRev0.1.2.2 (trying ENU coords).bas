...
Declare Function Coord2dbl(coord_str As String , Is_lat As Byte) As Double
...
'            Toggle Led
            ...
            If Strs(1) = "START" Then
              ...
              Do
                Get_gps
              Loop Until Have_gps_lock = 1

              Print #1 , Unit_name ; ":" ; Str(my_lat) ; ":" ; Str(my_lng) ; ":*"
              Do
                If New_remote_data = 1 Then
                  New_remote_data = 0
                  Astr = ""
...
Koth:
  Cls
  Do
    If New_gps_data = 1 Then Get_gps
    If New_remote_data = 1 Then Get_remote_data

    If Needs_redraw = 1 Then Redraw_koth
...
'      Lcdat 2 , 1 , Str(my_lng) + "  " , Orange , White
'      Lcdat 10 , 1 , Str(my_lat) + "  " , Blue , White
  ...
  Lcdat 112 , 1 , Str(enemy_x(1)) + "  " , Orange , White
  Lcdat 120 , 1 , Str(enemy_y(1)) + "  " , Blue , White
...
'    Zig_slp = 0
'    Waitms 13
    Print #1 , My_team ; "," ; My_x ; "," ; My_y ; "," ; My_heading ; ",*"
...
Sub Get_remote_data()
  ...
  B4 = Split(astr , Strs(1) , ",")                          'TEAM,NUMBER,LAT,LONG,HEADING,FIRING,*

  D1 = Coord2dbl(strs(3) , 1)
  D2 = Coord2dbl(strs(4) , 0)
  W1 = Get_xy(my_lat , My_lng , D1 , D2)
  B1 = Val(strs(2))

  If My_team = Strs(1) Then
    Friendly_x(b1) = High(w1)
    Friendly_y(b1) = Low(w1)
    Friendly_heading(b1) = Val(strs(5))
    If Strs(6) = "1" Then
  ...
  Lcdat 2 , 1 , Str(friendly_x(b1)) + "  " , Orange , White
