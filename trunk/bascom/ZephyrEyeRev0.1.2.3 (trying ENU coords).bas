...
Const Nnwest = 15
  'GPS
Const Earth_radius = 6378137.0                              'Radius of earth in meters
Const R = 6975174.98                                        'Radius of earth in yards
Const Yds_per_div = 121616                                  'Yards per longitude (or latitude)
  'Games
Const Cmd_coords = 1
Const Cmd_master_update = 2
Const Cmd_hit = 3
Const Cmd_out_of_bounds = 4
'*******************************************************************************
...
Declare Sub Get_gps()
Declare Sub Get_remote_data()
Declare Sub Redraw_koth()
Declare Function Coord2dbl(coord_str As String , Byval Is_lat As Byte) As Double
Declare Function Get_xy(lat1 As Double , Lng1 As Double , Lat2 As Double , Lng1 As Double) As Word
...
Dim Can_scan As Word                                        'Use a clock-cycle based timeout to avoid blocking statements during debounce
  'Games
Dim My_lng As Double
Dim My_lat As Double
...
Dim Friendly_x(10) As Byte
Dim Friendly_y(10) As Byte
...
Dim Enemy_x(10) As Byte
Dim Enemy_y(10) As Byte
...
Dim Temp_lats(6) As Double
Dim Temp_lngs(6) As Double
...
Dim Teama_start_lat As Double                               'Latitude of Team A's starting location
Dim Teama_start_lng As Double                               'Longitude of Team A's starting location
Dim Teamb_start_lat As Double                               'Latitude of Team B's starting location
Dim Teamb_start_lng As Double                               'Longitude of Team B's starting location
Dim Hill_lat As Double                                      'Latitude of hill's location
Dim Hill_lng As Double                                      'Longitude of hill's location
...
'Expect "START:[Loc_radius]:[View_friendly]:[View_enemy]:[Game_len]:[TO_len]:[AStrt_lat]:[AStrt_lng]:[BStrt_lat]:[BStrt_lng]:[Hill_lat]:[Hill_lng]:[TeamA_cnt]:[TeamB_cnt]:*"
'    or "CANCEL:*"
...
'V0.1.2:KOTH:Proto2:*
        Wait_for_start:
        Do
          If New_remote_data = 1 Then                       'Ex: START:10:1:0:1200:120:123.4567:123.4567:123.4567:123.4567:123.4567:123.4567:3:3:*
            New_remote_data = 0
            Astr = ""
            B2 = 1
            Do
            ...
            If Strs(1) = "START" Then
              ...
              Teama_start_lat = Val(strs(7))
              Teama_start_lng = Val(strs(8))
              Teamb_start_lat = Val(strs(9))
              Teamb_start_lng = Val(strs(10))
              Hill_lat = Val(strs(11))
              Hill_lng = Val(strs(12))
              ...
              Print #1 , My_team ; ":" ; My_number ; ":" ; My_lat ; ":" ; My_lng ; ":*"
              Do
...
Sub Setup_king_of_the_hill()
  ...
  For B1 = 1 To 5
    Teama_start_lat = Teama_start_lat + Temp_lats(b1)
    Teama_start_lng = Teama_start_lng + Temp_lngs(b1)
  Next B1
  Teama_start_lat = Teama_start_lat / 5
  Teama_start_lng = Teama_start_lng / 5
  ...
  For B1 = 1 To 5
    Teamb_start_lat = Teamb_start_lat + Temp_lats(b1)
    Teamb_start_lng = Teamb_start_lng + Temp_lats(b1)
  Next B1
  Teamb_start_lat = Teamb_start_lat / 5
  Teamb_start_lng = Teamb_start_lng / 5
  ...
  For B1 = 1 To 5
    Hill_lat = Hill_lat + Temp_lats(b1)
    Hill_lng = Hill_lng + Temp_lngs(b1)
  Next B1
  Hill_lat = Hill_lat / 5
  Hill_lng = Hill_lng / 5
  ...
  Teama_count = 1                                           'Team A player count
  Teamb_count = 0                                           'Team B player count
  Zig_slp = 0
  Waitms 15
  Wait_for_start_conf:
  Clear Serialin
...
'        Line(0 , 9) -(128 , 9) , Black
  ...
  Astr = Astr + Str(teama_start_lat) + ":" + Str(teama_start_lng) + ":" + Str(teamb_start_lat) + ":" + Str(teamb_start_lng) + ":"
  Astr = Astr + Str(hill_lat) + ":" + Str(hill_lng) + ":" + Str(teama_count) + ":" + Str(teamb_count) + ":*"
...
Function Check_start_points() As Byte                       'If we return 1, it's ok, if zero, someone isn't at their starting location
  ...
  Lb2 = Lb2 - 1
  Do                                                        'Get coordinates from each player
    If New_remote_data = 1 Then                             'Will look like: "B:1:123.4567:12.3456:*"
      Astr = ""
      B2 = 1
      Do
        B1 = Waitkey(#2)
...
Koth:
  ...
  Do
    If New_gps_data = 1 Then
      Get_gps
      If Return_valid = 1 Then
        W1 = Get_xy(my_lat , My_lng , Hill_lat , Hill_lng)
        If Low(w1) < Loc_radius And High(w1) < Loc_radius Then
        ...
        'Example String: "1:A:1:1234567:1234567:270:1:0:*"  (Cmd "1" = Coordinate Update)
        Astr = "1:" + My_team + ":" + Str(my_number) + ":" + Str(my_lat) + ":" + Str(my_lng) + ":" + Str(my_heading) + ":" + Str(am_on_hill) + ":" + Str(am_hit) + ":*"
    ...
    If New_remote_data = 1 Then                             'Get remote data
      ...
      B4 = Split(astr , Strs(1) , ":")                      'Should look like "1:A:1:12.34567:123.4567:270:1:0:*"
      B3 = Val(strs(3))                                     'Contains player number

      Temp_lats(1) = Coord2dbl(strs(4) , 1)
      Temp_lngs(1) = Coord2dbl(strs(5) , 0)
      W1 = Get_xy(my_lat , My_lng , Temp_lats(1) , Temp_lngs(1))

      If Strs(2) = My_team Then                             'Make sure we modify the right team
        Friendly_x(b3) = High(w1)                           'Longitude
        Friendly_y(b3) = Low(w1)                            'Latitude
        Friendly_headings(b3) = Val(strs(6))                'Heading
        'Friendlies_firing = ...
        B4 = Val(strs(7))                                   'Store if this friendly is "on the hill"
        Friendlies_on_hill.b3 = B4
        B4 = Val(strs(8))                                   'Store if this friendly is currently hit
        W1 = Friendlies_hit
      ...
      Else
        Enemy_x(b3) = High(w1)                              'Longitude
        Enemy_y(b3) = Low(w1)                               'Latitude
        Enemy_headings(b3) = Val(strs(6))                   'Heading
        'Enemies_firing = ...
        B4 = Val(strs(7))                                   'Store if this enemy is "on the hill"
        Enemies_on_hill.b3 = B4.0
        B4 = Val(strs(8))                                   'Store it if this enemy is currently hit
        W1 = Enemies_hit
    ...
    If Can_scan > 3500 Then                                 ' Koth_sw_scan Else Incr Can_scan
...
Sub Koth_redraw()
  ...
  For B1 = 1 To Friendly_count
    If B1 <> My_number Then
      Draw_arrow Old_friendly_y(b1) , Old_friendly_x(b1) , White , Old_friendly_headings(b1)
    End If
  Next B1

  'Clear enemies from screen
  For B1 = 1 To Enemy_count
    Draw_arrow Old_enemy_y(b1) , Old_enemy_x(b1) , White , Old_enemy_headings(b1)       'Draw a white arrow over the last arrow
  Next B1

  'Update zoom level
  B1 = Zoom_factor * 100
  Lcdat 2 , 1 , Str(b1) + "m" , Dark_gray , 2 , 1

  'Update game time remaining
  B1 = Game_remaining \ 60
  B2 = Game_remaining Mod 60
  If B2 > 9 Then
    Lcdat 120 , 1 , Str(b1) + ":" + Str(b2) , Black , White
  Else
    Lcdat 120 , 1 , Str(b1) + ":0" + Str(b2) , Black , White
  End If
  ...
  'Update timeout remaining, if any
  If Am_hit = 1 Then
    B1 = Timeout_remaining \ 60
    B2 = Timeout_remaining Mod 60
    If B2 > 9 Then
      Lcdat 48 , 120 , Str(b1) + ":" + Str(b2) , Black , White
    Else
      Lcdat 48 , 120 , Str(b1) + ":0" + Str(b2) , Black , White
    End If
  End If
...
'        Factor_zoom
        ...
        Draw_arrow Y , X , Color , Friendly_headings(b1)
      End If
    Next B1
    ...
    If My_team = "A" Then
      ...
      Cur_y = Teama_start_loc_y - My_y
    Else
      Cur_x = Teamb_start_loc_x - My_x
      Cur_y = Teamb_start_loc_y - My_y
    End If
    Cur_x = Cur_x + 64
    Cur_y = Cur_y + 64
'    Factor_zoom
...
'      Factor_zoom
      ...
      Draw_arrow Y , X , Color , Enemy_headings(b1)
    Next B1
...
'    Print #1 , "Long: " ; Strs(6)
    ...
    If Strs(3) = "A" Then
      ...
      'Get Latitude
      If Strs(4) <> "" Then
        My_lng = Coord2dbl(strs(4) , 1)                     'Convert the latitude to a double and store in My_lat
        If Strs(5) = "S" Then My_lng = My_lng * -1
      Else
        Have_gps_lock = 0
      End If
      ...
      'Get Longitude
      If Strs(6) <> "" Then
        My_lat = Coord2dbl(strs(6) , 0)                     'Convert the longitude to a doube and store in My_lng
        If Strs(7) = "W" Then My_lat = My_lat * -1
      Else
        Have_gps_lock = 0
      End If
...
'    Waitms 13
    ...
    'CMD,TEAM,NUMBER,LAT,LONG,HEADING,FIRING,*
    Print #1 , "1," ; My_team ; "," ; My_number ; "," ; My_lat ; "," ; My_lng ; "," ; My_heading ; ",*"
...
Sub Get_remote_data()
  ...
  B4 = Split(astr , Strs(1) , ",")                          'CMD,TEAM,NUMBER,LAT,LONG,HEADING,FIRING,*

  D1 = Coord2dbl(strs(4) , 1)
  D2 = Coord2dbl(strs(5) , 0)
  W1 = Get_xy(my_lat , My_lng , D1 , D2)
  B1 = Val(strs(3))

  If My_team = Strs(2) Then
    Friendly_x(b1) = High(w1)
    Friendly_y(b1) = Low(w1)
    Friendly_heading(b1) = Val(strs(6))
    If Strs(7) = "1" Then
      Friendly_firing.b1 = 1
    Else
      Friendly_firing.b1 = 0
    End If
  Else
    Enemy_x(b1) = High(w1)
    Enemy_y(b1) = Low(w1)
    Enemy_heading(b1) = Val(strs(6))
    If Strs(7) = "1" Then
      Enemy_firing.b1 = 1
    Else
      Enemy_firing.b1 = 0
...
'  Lcdat 2 , 1 , Str(enemy_x(b1)) + "  " , Orange , White

  If _rs_bufcountr0 < 20 Then New_remote_data = 0
End Sub
'*******************************************************************************
...
Function Coord2dbl(coord_str As String , Byval Is_lat As Byte) As Double
  Local Loc_d1 As Double , Loc_d2 As Double , Loc_b1 As Byte
  If Is_lat = 1 Then
  ...
  Loc_d1 = Val(split_strs(1))
  Loc_d2 = Val(split_strs(2))
  Loc_d2 = Loc_d2 / 60
  If Dbl1 > 0 Then
    Coord2dbl = Loc_d1 + Loc_d2
  Else
    Coord2dbl = Loc_d1 - Loc_d2
  End If
End Function
...
'*******************************************************************************
Function Get_xy(lat1 As Double , Lng1 As Double , Lat2 As Double , Lng1 As Double) As Word
  Local Loc_d3 As Double , Loc_d4 As Double
  Local Loc_x As Long , Loc_y As Long
  Local Loc_b1 As Byte , Loc_b2 As Byte
...
'  Print "2: " ; Str(dbl_lat2) ; " " ; Str(dbl_lng2)
  ...
  Loc_d3 = Dbl_lat1 - Dbl_lat2
  Loc_d3 = Loc_d3 * Yds_per_div
  Loc_d3 = Loc_d3 + 64
  Loc_x = Loc_d3                                            'Demote from double to long
  Loc_x = Loc_x + 64
  Loc_b1 = Loc_x                                            'Demote from long to byte

  Loc_d3 = Dbl_lng1 - Dbl_lng2
  Loc_d3 = Loc_d3 * Yds_per_div
  Loc_d4 = Dbl_lat1 / 57.3
  Loc_d4 = Cos(loc_d4)
  Loc_d3 = Loc_d3 * Loc_d4
  Loc_y = Loc_d3
  Loc_b2 = Y


  Get_xy = Makeint(loc_b1 , Loc_b2)
End Function
'*******************************************************************************
